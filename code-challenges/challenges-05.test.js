...
const templateWithJQuery = () => {
  // Solution code here...
  starWarsPeople.forEach((amount) =>{
    let objClone=$('#template').clone();
    $('main').append(objClone);
    objClone.find('h2').text(amount.name);
    objClone.find('h3').text(amount.height);
    objClone.find('p').text(amount.eye_color);
...
const wordsToCharList = (arr) => {
  ...
  for(let i=0 ; i< arr. length ;i++){
    let arrsep = arr.slice(i,i+1);
    sepratedarr.push(arrsep);
  }
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach((item)=>{
    ...
    let final =value.slice(value.indexOf(' ')+1);
    result.push(final);
  });
